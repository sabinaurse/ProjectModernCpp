...
void GameManager::StartMatch(const std::vector<Player*>& playersForMatch) {
...

    {
        ...
        for (auto* player : playersForMatch) {
            newGame->AddPlayer(std::make_unique<Player>(std::move(*player)));
        ...
        std::cout << "[GameManager] Jocul cu ID  " << currentGameId << " a început și a fost adăugat în lista jocurilor active." << std::endl;
    }
}
